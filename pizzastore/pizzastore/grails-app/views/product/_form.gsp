<%@ page import="pizzastore.Product" %>
...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'status', 'error')} required">
    ...
    <g:select name="status" from="${productInstance.constraints.status.inList}" required=""
              value="${productInstance?.status}" valueMessagePrefix="product.status"/>
...
<div class="fieldcontain ${hasErrors(bean: productInstance, field: 'tags', 'error')} ">
    ...
    <g:select name="tags" from="${pizzastore.Tag.list()}" multiple="multiple" optionKey="id" size="5"
              value="${productInstance?.tags*.id}" class="many-to-many"/>
